...
def train(model_name, gpu_id):
    ...
    for step in range(0, n_iters):
        ...
        if step > 0 and step % params['model_save_interval'] == 0:
            model.save_weights((network_dir + '/' + 'weights_model_gan_improved'+ '.h5'))
#     model.save(network_dir + '/' + 'final_model'+ '.h5')
    


if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Need model name and GPU as command line arguments.")
    else:
