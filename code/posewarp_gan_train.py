...
def train(model_name, gpu_id):
    ...
    generator.load_weights(network_dir + '/' + 'weights_model_aws'+ '.h5')
        #'../models/vgg_100000.h5')
    ...
    for step in range(n_iters):
        ...
        if step < 5:
            util.printProgress('Iteration: ', step, 'Discriminator: ', [0, d_loss])
            step += 1
            continue
        ...
        util.printProgress('Iteration ',step, 'Generation and Discriminator loss', [g_loss[1], d_loss])
        
        if step % params['model_save_interval'] == 0 and step > 0:
            generator.save_weights((network_dir + '/' + 'weights_gan_model'+ '.h5'))

#         if step % params['model_save_interval'] == 0 and step > 0:
      
#     save(network_dir + '/' + str(step) + '.h5')
