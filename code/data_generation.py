...
#     print(vids)
    n_vids = len(vids)
    vid_info = []
    info_name = data_dir + '/info/YouTube_Pose_dataset.mat'
    pickle_in = open(data_dir +"/info/YouTube_dict.pickle",'rb')
    youtube_dict = pickle.load(pickle_in)
    info = sio.loadmat(info_name)
    frame_idx = info['data']['frameids'][:]
    frame_idx = frame_idx.reshape((50,1))
#     print(frame_idx.shape)
    box = info['data']['crop'][:]
    box = box.reshape((50,1))
    x = info['data']['locs'][:]
    x = x.reshape((50,1))
    scale_val = info['data']['scale'][:]
    scale_val = scale_val.reshape((50,1))


    for i in range(n_vids):
        path, vid_name = os.path.split(vids[i])
        index = youtube_dict[vid_name]
        vid_info.append([box[index-1], x[index-1], vids[i],scale_val[index-1],frame_idx[index-1]])
    return vid_info


def read_frame(vid_name, image_num, x,frame_num):
    img_name = os.path.join(vid_name, str(image_num) + '.jpg')
    if not os.path.isfile(img_name):
        img_name = os.path.join(vid_name, str(image_num) + '.png')
...
def warp_example_generator(vid_info_list, param, do_augment= True, return_pose_vectors=False,mode = 'train'):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            vid_bbox = vid_info_list[vid][0]
            vid_bbox = vid_bbox[0][0]
            vid_x = vid_info_list[vid][1][0]
            vid_path = vid_info_list[vid][2]
            vid_scale = vid_info_list[vid][3][0][0]
            vid_frame = vid_info_list[vid][4][0]
...
#             print(vid_frame.shape)
            pos[0] = (vid_bbox[0] + vid_bbox[2] / 2.0)
            pos[1] = (vid_bbox[1] + vid_bbox[3] / 2.0)
            ...
            I0, joints0= read_frame(vid_path, vid_frame[0,frames[0]],vid_x,frames[0])
            I1, joints1 = read_frame(vid_path, vid_frame[0,frames[1]], vid_x,frames[1])
#             print(frames[0])
#             print(frames[1])
...
def create_feed(params, data_dir, mode, do_augment=True, return_pose_vectors=False, transfer=False):
    ...
    if transfer:
        ...
        feed = warp_example_generator(vid_info_list, params, do_augment, return_pose_vectors,mode)
...
#     print(I.shape)
    ...
    scale_x = (img_width/(xshape))
    scale_y  = (img_height/(yshape))
    joints[:,0] = joints[:,0] * scale_y
    joints[:,1] = joints[:,1] * scale_x
...
def make_limb_masks(limbs, joints, img_width, img_height):
    ...
    sigma_perp = np.array([11, 11, 11, 11, 11]) ** 2 #, 11, 11, 11, 11, 13
    ...
    for i in range(n_limbs):
        ...
        if n_joints_for_limb == 3:
            p_top = p[0,:]
            p_bot = np.mean(p[1:2, :], axis=0)
